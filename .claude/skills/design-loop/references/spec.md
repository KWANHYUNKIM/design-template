...
21. **여백을 갈랐다고 말하려면 390에서 재라** — `@media(max-width:900px){.sheet,.sheet-n{좌우 같은 값}}`
    한 줄이 데스크톱의 7/7 교대를 통째로 지운다. 문서의 주 뷰포트는 390이다.
    `--probe` 의 `rhythm`/`rhythmWarn` 이 잰다 (RUN33 · 같은 결함이 RUN30·31에도 있었다)
22. **고정 `viewBox` 안의 글자는 토큰화로 안 풀린다** — 실렌더 = `font-size × (렌더폭 ÷ viewBox폭)`.
    폭 구간별로 **사용자 단위를 올려야** 한다. `--probe` 의 `svgIssues` 가 12px 미만을 잡는다.
    그리고 SVG 는 **상태를 바꾼 뒤에도** `getBBox()` 가 viewBox 안인지 재라 (RUN25 §85 · RUN33)
23. **조작형 리드아웃은 상수가 아니라 도형에서 파생시킬 것** — 값을 적어 두면 계산인지 구별되지 않을
    뿐 아니라 **틀려도 모른다.** RUN33은 `path` 의 `d` 를 파싱하도록 바꾸자 그 자리에서
    인덱스 오류가 드러났다(1:3 이 1:12 로 출력)
24. **명시 `grid-column` 을 가진 자식 뒤에 자동배치 자식을 두지 말 것** — sparse 배치가 커서를
    명시 칸 뒤로 보내 **마지막 자식이 다음 행 1열(트랙 하나 폭)로 떨어진다.**
    RUN34는 세 자리 점수가 「29 / 0」으로 쪼개졌다. 두 칸 이상 쓰는 행은 **전부 `grid-area` 로 명시.**
    `--probe` 의 `gridFlow` 가 잰다
25. **조작형의 기본 상태를 정적 마크업으로 먼저 그려 둘 것** — JS 가 만들어 넣는 내용이 마크업에 없으면
    스크립트가 꺼진 브라우저에서 그 섹션이 통째로 사라진다. `.rise{opacity:0}` 만 고친 것으로는 부족하다
    (함정 16과 별개). `--probe` 의 `noJS` 는 이제 **보이는 텍스트 길이 비**로 잰다 (RUN34)
26. **`opacity` 로 흐리지 말 것** — 합성 후 대비가 무너진다. RUN34의 `opacity:.55` 는
    `--mut` 를 2.60:1(AA 미달)로 만들었다. **색을 직접 낮추고 그 값의 대비를 실측**할 것
27. **접기 미디어쿼리 안에서는 기본 규칙의 특이도를 되받아라** — `tbody tr:last-child td`
    는 **(0,1,3)** 이고 `.fold td` 는 **(0,1,1)** 이다. 미디어쿼리는 특이도를 안 올리므로
    기본 규칙이 이긴다. RUN35는 마지막 카드만 3px 잉크 괘선으로 조각났다.
    `.fold tbody tr:last-child td{border-bottom:0}` 처럼 **같은 셀렉터로 덮을 것.**
    그리고 **접힌 표는 마지막 카드를 따로 눈으로 볼 것** — 나머지와 형태가 다르다
28. **수치+단위가 들어가는 좁은 열에 `em` 폭을 쓰지 말 것** — RUN35의 `minmax(0,6em)` 은
    「6,000 kg」을 수치와 단위 두 줄로 쪼갰다. **오버플로가 아니라서 검사기에 안 걸린다.**
    `max-content` + `white-space:nowrap` 으로 두고 **390에서 확대해 눈으로 확인**할 것
29. **바닥·천장 근처를 가리키는 표시물은 「표시선」과 「라벨」을 분리할 것** — 한 덩어리로 두면
    자기 높이의 절반만큼 끝까지 못 가서, 서로 다른 값이 **같은 자리로 클램프**된다.
    RUN35는 지상 1.5m와 4.5m가 390에서 똑같이 붙었다. 3px 선은 정확한 좌표에,
    라벨은 그 위에 가둔다
30. **대비 주석은 계산해서 적을 것 — 눈으로 어림하면 반드시 틀린다.** RUN35는 9.11·12.6이라
    적고 실제는 9.84·13.97이었다. `:root` **밖**에 선언한 색도 예외가 아니다.
    `verify.mjs` 의 `contrast-annotation-true` 가 이제 **주석값을 실측과 대조**한다
31. **`<figure>`·`<blockquote>`·`<dd>` 는 UA 가로 마진 40px 을 갖는다 — 반드시 리셋할 것.**
    `*{box-sizing}` 만 하고 넘어가면 **그 요소만** 판형 밖으로 40px 밀린다.
    `.figwrap{margin-top:…}` 처럼 클래스에 세로 마진만 주는 것은 리셋이 아니다.
    RUN25에서 지적됐고 RUN36에서 재발했으며, 검사로 옮기니 과거 여섯 장에도 있었다.
    `figure,blockquote,dd{margin:0}` — `verify.mjs` 의 `ua-margin-reset` 이 잡는다
32. **폴백에서 끄는 트랜지션·애니메이션은 실제로 존재해야 한다.** `X{transition:none}` 을 써 놓고
    `X` 에 `transition` 선언이 없으면 **「폴백을 챙겼다」는 착각만** 남는다. RUN36은
    reduced-motion 과 `.capture` 양쪽에서 없는 트랜지션 두 개를 껐다.
    그리고 `.capture` 와 `.js` 는 **둘 다 `<html>`** 에 붙으므로 `.capture .js X` 는 영원히 안 맞는다.
    `verify.mjs` 의 `dead-fallback` 이 잡는다
33. **2열에서 `position:sticky` 는 「짧은 칸」에 걸어야 한다.** 긴 쪽에 걸면 아무 일도 안 일어난다.
    RUN36은 404px 곁줄에 sticky 를 걸고 134px 주 칼럼 아래를 270px 비웠다.
    더 나은 답은 애초에 **높이를 맞추는 것** — 문단 하나를 짧은 칸으로 옮기면 대개 붙는다.
    `--probe` 의 `trackMismatch` 와 `rhythmWarn` 이 둘 다 잰다
34. **한 패널 안의 계량 단위를 섞지 말 것.** RUN36은 네 행이 「글자 수」인데 한 행만
    「덩어리 수」를 세어 **화면의 한자 5자를 「3개」로** 표시했다(`.sino` span 개수).
    파생값이라도 **무엇을 세는지가 이웃 행과 같은 종류**여야 한다 (함정 9의 파생값 판본)
